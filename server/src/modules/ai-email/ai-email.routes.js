...
router.post(
  '/hostinger/webhook',
  webhookLimiter,
  createWebhookPayloadGuard(),
  asyncHandler(handleHostingerWebhook)
)
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await approveAndQueueSend(
      req.params.replyId,
      req.user.id,
      edited_body
    )
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await rejectReply(
      req.params.replyId,
      req.user.id,
      reason
    )
