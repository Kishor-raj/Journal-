...
router.get('/dashboard', authenticate, requireRole('admin', 'editor'), async (req, res) => {
...
router.get('/approvals', authenticate, requireRole('admin', 'editor'), async (req, res) => {
  const { page, limit } = req.query
  const result = await getPendingApprovals({ page, limit })
  res.json(result)
})

router.get('/approvals/:replyId', authenticate, requireRole('admin', 'editor'), async (req, res) => {
  const detail = await getApprovalDetail(req.params.replyId)
  if (!detail) return res.status(404).json({ error: 'Reply not found' })
  res.json(detail)
})

router.post('/approvals/:replyId/approve', authenticate, requireRole('admin', 'editor'), approvalLimiter, async (req, res) => {
...
router.post('/approvals/:replyId/reject', authenticate, requireRole('admin', 'editor'), approvalLimiter, async (req, res) => {
...
router.get('/timeline/:emailId', authenticate, requireRole('admin', 'editor'), async (req, res) => {
